package com.deadrudolph.uicomponents.view.textfield.core.saver
...
import com.deadrudolph.uicomponents.view.textfield.core.saver.AnnotationType.Paragraph
import com.deadrudolph.uicomponents.view.textfield.core.saver.AnnotationType.Span
import com.deadrudolph.uicomponents.view.textfield.core.saver.AnnotationType.Url
import com.deadrudolph.uicomponents.view.textfield.core.saver.AnnotationType.VerbatimTts
...
import com.deadrudolph.uicomponents.view.textfield.core.string.AnnotatedString.Range
...
internal val AnnotatedStringSaver = Saver<AnnotatedString, Any>(
    ...
    restore = {
        ...
        val spanStylesOrNull: List<Range<SpanStyle>>? =
            restore(list[1], AnnotationRangeListSaver)
        val paragraphStylesOrNull: List<Range<ParagraphStyle>>? =
...
private val AnnotationRangeListSaver = Saver<List<Range<out Any>>, Any>(
    save = {
        it.fastMap { range ->
    ...
    restore = {
        ...
        val list = it as List<Any>
        list.fastMap { item ->
            val range: Range<out Any> = restore(item, AnnotationRangeSaver)!!
            range
        }
    }
)
...
private val AnnotationRangeSaver = Saver<Range<out Any>, Any>(
    save = {
        val marker = when (it.item) {
            is ParagraphStyle -> Paragraph
            is SpanStyle -> Span
            is VerbatimTtsAnnotation -> VerbatimTts
            is UrlAnnotation -> Url
            else -> AnnotationType.String
        }

        val item = when (marker) {
            Paragraph -> save(it.item as ParagraphStyle, ParagraphStyleSaver, this)
            Span -> save(it.item as SpanStyle, SpanStyleSaver, this)
            VerbatimTts -> save(
    ...
    restore = {
        ...
        when (marker) {
            Paragraph -> {
                ...
                Range(item = item, start = start, end = end, tag = tag)
            }
            Span -> {
