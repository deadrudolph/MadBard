...
internal fun AndroidTextPaint.applySpanStyle(
    ...
    if (style.textGeometricTransform != null &&
        style.textGeometricTransform != emptyTextTransform
    ) {
...
private fun generateFallbackSpanStyle(
    ...
    return if (!hasLetterSpacing && !hasBackgroundColor && !hasBaselineShift) {
        ...
        SpanStyle(
            letterSpacing = if (hasLetterSpacing) {
                letterSpacing
            } else {
                TextUnit.Unspecified
            },
            background = if (hasBackgroundColor) {
                background
            } else {
                Color.Unspecified
            },
            baselineShift = if (hasBaselineShift) {
                baselineShift
            } else {
                null
            }
...
@OptIn(ExperimentalTextApi::class)
internal fun AndroidTextPaint.setTextMotion(
...
internal fun TextStyle.isIncludeFontPaddingEnabled(): Boolean {
    return platformStyle?.paragraphStyle?.includeFontPadding ?: true
}