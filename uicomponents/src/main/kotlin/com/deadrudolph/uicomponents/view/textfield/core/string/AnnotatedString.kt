...
import com.deadrudolph.uicomponents.view.textfield.core.string.AnnotatedString.Builder
...
@Immutable
class AnnotatedString constructor(
    val text: String,
    val spanStylesOrNull: List<Range<SpanStyle>>? = null,
    val paragraphStylesOrNull: List<Range<ParagraphStyle>>? = null,
    val annotations: List<Range<out Any>>? = null
) : CharSequence {
 ...
 */
internal fun AnnotatedString.toUpperCase(localeList: LocaleList = LocaleList.current): AnnotatedString {
 ...
 */
internal fun AnnotatedString.toLowerCase(localeList: LocaleList = LocaleList.current): AnnotatedString {
 ...
 */
internal fun AnnotatedString.capitalize(localeList: LocaleList = LocaleList.current): AnnotatedString {
 ...
 */
internal fun AnnotatedString.decapitalize(localeList: LocaleList = LocaleList.current): AnnotatedString {
    return transform { str, start, end ->
        if (start == 0) {
 ...
 */
internal inline fun <R : Any> Builder.withStyle(
    style: SpanStyle,
    block: Builder.() -> R
): R {
 ...
 */
@ExperimentalTextApi
internal inline fun <R : Any> Builder.withAnnotation(
    tag: String,
    annotation: String,
    crossinline block: Builder.() -> R
): R {
 ...
 */
internal inline fun buildAnnotatedString(builder: (Builder).() -> Unit): AnnotatedString =
